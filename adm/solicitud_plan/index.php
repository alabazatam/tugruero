...
function executeListJson($values)
{
	...
	if(count($list_json)>0)
	{
		foreach ($list_json as $list)
		{
			...
			if($list['NombreVendedor']=='tugruero.com'){
				...
				$cuadro_rcv_carnet = $list['PolizaAsistir'].'_carnet.pdf';
			}
				...
				if($plan_tugruero == true and $plan_rcv == true){
					if($list['NombreVendedor'] == "CCCT"){
						$array_json['data'][] = array(
							...
							<li><a href="'.full_url.'/web/files/Cuadros/'.$cuadro_rcv_carnet.'" class="" target="_blank" title="Imprimir RCV"><i class="fa fa-file-pdf-o"></i> Carnet RCV</a></li>
					...
					if($list['NombreVendedor'] == "CCCT"){
						$array_json['data'][] = array(
							...
							<li><a href="'.full_url.'/adm/solicitud_plan/index.php?action=edit&idSolicitudPlan='.$idSolicitudPlan.'"> <i class="fa fa-edit"></i> Editar</a></li>
							<li><a href="'.full_url.'/web/files/Cuadros/'.$list['NumProducto'].'.pdf" class="" target="_blank" title="Imprimir Cuadro"><i class="fa fa-file-pdf-o"></i> Cuadro producto</a></li>
							<li><a href="'.full_url.'/web/files/Cuadros/'.$list['NumProducto'].'_carnet.pdf" class="" target="_blank" title="Imprimir Carnet"><i class="fa fa-file-pdf-o"></i> Carnet producto</a></li>
	...
	function executePrecioTugruero($values = null,$errors = array())
	{
		...
		if($idPlan!=''){
			...
			if(count($recarga)>0){
				$array["Kilometraje"] = $recarga["Kilometraje"];
				$array["CantidadServicios"] = $recarga["CantidadServicios"];
			}
